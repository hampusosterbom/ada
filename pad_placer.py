...
# Helper Functions: These functions parse configs, compute deficits, plot histograms, generate candidates, and precompute histograms.

def parse_cfg(path):
    """Parse ALMA config file into DataFrame."""
    rows = []
...
def plot_normalized_histogram(baselines, R_max, nbins=50, title=None, filename=None, alpha=0.7, figsize=(6,4)):
    """Plot normalized baseline histogram."""
    u = baselines / R_max
    edges = np.linspace(0, 1, nbins + 1)
    plt.figure(figsize=figsize)
    plt.hist(u, bins=edges, density=True, alpha=alpha, edgecolor='k', linewidth=0.5)
...
def plot_angular_deficit(hist, deficit, bins, save_path=None):
    """Polar bar plot of angular deficits."""
    centers = (bins[:-1] + bins[1:]) / 2
    width = bins[1] - bins[0]
    fig = plt.figure(figsize=(5, 5))
    ax = fig.add_subplot(111, projection='polar')
    ax.bar(centers, np.clip(deficit, 0, None), width=width, bottom=0, alpha=0.7)
...
def precompute_hist_ec_2d(existing_pts, cand_pts, r_bins, theta_bins, ex_weights):
    """Precompute 2D histogram for existing-to-candidate baselines."""
    N_r = len(r_bins) - 1
...
# Main Logic: Refactored into sub-functions for loading data, optimization, writing outputs, and generating plots.

def load_data(args):
    """Load existing pads, regions, and setup CRS."""
    existing_df = parse_cfg(args.cfg_path)
    existing_df['weight'] = 1.0
    existing_pts = existing_df[['x', 'y']].values

    # Find starting NP counter
...
def optimize_pads(args, existing_df, existing_pts, ex_weights, ctr_start, allowed_area, region_df):
    """Perform greedy optimization to select new pads."""
    ...
    logging.info(f"Total candidates: {len(cand_df)} | Region: {np.sum(cand_df['type'] == 'region')} | Inner: {np.sum(cand_df['region'] == 'inner')}")

    cand_pts = cand_df[['x', 'y']].values
    cand_region = cand_df['region'].values
    cand_rad = np.hypot(cand_pts[:, 0], cand_pts[:, 1])  # Precompute once
    ...
    base_pts = existing_df[['x', 'y']].values  # True existing only
    ...
    row_ee = np.where(valid_ee)[0]  # Simplified
    weights_ee = ex_weights[row_ee]
    ee_hist = np.bincount(cells_ee, weights=weights_ee, minlength=N_cells) / 2.0

    cable_dists = np.linalg.norm(cand_pts - existing_pts[0], axis=1)  # Base at first existing
    ...
    return new_df, existing_df, cand_df, sel_df, cand_pts, sel, args.fixed_r_max
...
def generate_plots(new_df, existing_df, fixed_r_max):
    ...
    bl_bins = np.linspace(0, fixed_r_max, 65)
    ...
    plot_normalized_histogram(
        ...
        R_max=fixed_r_max,
        nbins=65,
        title="Norm. Baseline distribution",
...
def main():
    ...
    new_df, existing_df, cand_df, sel_df, cand_pts, sel, fixed_r_max = optimize_pads(
