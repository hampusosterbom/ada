...
def compute_angular_deficit(existing_pts, new_pts, n_theta=16):
    """
    #Compute angular histogram and deficit for baselines from existing to new points.
    #"""
...
def plot_normalized_histogram(baselines, R_max, nbins=50,
    ...
    """
    baselines : 1D array of distances (m)
    R_max     : scalar, the max baseline distance used for normalization
    nbins     : number of bins over [0,1]
    title     : optional string for the plot title
    filename  : if given, the plot is saved to this path (e.g. 'hist_norm.png')
...
#def plot_angular_deficit(hist, deficit, bins, save_path=None):
    #"""Polar bar plot of the deficits (positive = under-served)."""
...
def precompute_hist_ec_2d(existing_pts, cand_pts, r_bins, theta_bins, ex_weights):
    """
    Compute a weighted 2D histogram of distances & angles from each candidate
    to all existing pads.

    Parameters
    ----------
    existing_pts : array-like, shape (N_existing, 2)
        Coordinates (x, y) of existing pads.
    cand_pts : array-like, shape (N_cand, 2)
        Coordinates (x, y) of candidate pads.
    r_bins : array-like, shape (N_r+1,)
        Radial bin edges.
    theta_bins : array-like, shape (N_theta+1,)
        Angular bin edges (radians, in [0, 2π]).
    ex_weights : array-like, shape (N_existing,)
        Weight for each existing pad; used to weight each “hit” in the histogram.

    Returns
    -------
    hist_ec_2d : ndarray, shape (N_cand, N_r * N_theta)
        For each candidate j, a flattened histogram over (r, θ) cells,
        where each existing pad contributes its weight to the appropriate cell.
    """
    N_r = len(r_bins) - 1   
    ...
    d_ec_raw = cdist(existing_pts, cand_pts)                           # (N_exist, N_cand)
    ...
    # bin-indices for each pair
    ir_ec = np.digitize(d_ec_raw, r_bins) - 1                          # radial bin
    it_ec = np.digitize(phi_ec_raw, theta_bins) - 1                    # angular bin
    valid_ec = (
        (ir_ec >= 0) & (ir_ec < N_r) &
        (it_ec >= 0) & (it_ec < N_theta)
    )
    cell_ec = ir_ec * N_theta + it_ec                                  # flatten to [0, N_r*N_theta)

    # build weighted histograms
    hist_ec_2d = np.zeros((N_cand, N_cells), dtype=float)
    for j in range(N_cand):
        # indices of existing pads that fall into any valid cell for candidate j
...
def main():
    ...
    parser.add_argument('--cfg_path', default="C:/Users/hampu/Downloads/alma.cycle11.10s.cfg") #"C:/Users/hampu/Downloads/alma.cycle11.10s.cfg"#"C:/Users/hampu/OneDrive/Skrivbord/alma2040/customCfgs/presentation/alma.cycle11.10.30_OSFV1.cfg"
    parser.add_argument('--kml_path', default="C:/Users/hampu/OneDrive/Skrivbord/alma2040/configREAL/v5.kml")#"C:/Users/hampu/OneDrive/Skrivbord/alma2040/customCfgs/alma.cycle11.10.157_v18.cfg"
    ...
    parser.add_argument('--fixed_r_max', type=float, default=50000.0, help="Fixed maximum for radial bins to ensure consistency across runs")# 44000.0
    parser.add_argument('--ratio_penalty_weight', type=float, default=0, help="Weight for the soft ratio penalty")
    parser.add_argument('--transition_width', type=float, default=5000.0, help="Width for sigmoid transition in soft ratio enforcement")
    parser.add_argument('--density_radius', type=float, default=1700.0, help="Radius for local density penalty (anti-clustering)") 
    parser.add_argument('--density_weight', type=float, default=0.35, help="Weight for local density penalty")
    args = parser.parse_args()
    args.include_reuse = False#True
    # Load existing pads
    existing_df = parse_cfg(args.cfg_path)
    existing_df['weight'] = 1.0
    existing_pts = existing_df[['x', 'y']].values
    
    base_pts = existing_pts  # Set base_pts to true existing only
    ...
    # Load allowed areas
    regions_gdf = gpd.read_file(args.kml_path, layer=args.poly_layer, driver="KML").to_crs(crs_loc)
    if regions_gdf.empty:
        raise ValueError("No regions loaded from KML.")
    # Load allowed areas from multiple layers
    poly_layers =['Regions OSF']#["CarlosAllowed", "extraRegions"] # ["CarlosAllowed", "extraRegions"] #["center1"]#["CarlosAllowed", "extraRegions"]  # Add more if needed
    polys = []
    ...
    # Load region pads
    region_layers = ["Central Cluster", "iner cluster", "W", "P", "S"]  # Corrected typo
    region_pads = []
    ...
    weights = all_existing['weight'].values if 'weight' in all_existing else np.ones(len(all_existing))
    ...
    cand_region = cand_df['region'].values  # Define here for use in loop
    cand_rad = np.hypot(cand_pts[:, 0], cand_pts[:, 1])  # Radial distances for candidates
    radial_boost = np.ones(len(cand_pts))
    # scale down cost by 20% in the 2–4 km ring (i.e. boost selection)
    #mask_ring = (cand_rad >= 1000) & (cand_rad <= 12000)
    #radial_boost[mask_ring] = 0.8 
    #mask_inner = cand_rad < 2000
    #radial_boost[mask_inner] = 1  # Mild inner boost for density
    #mask_mid1 = (cand_rad > 1500) & (cand_rad < 2500)
    #radial_boost[mask_mid1] = 0.5
    
    #mask_mid = (cand_rad > 4000) & (cand_rad < 6000)
    #radial_boost[mask_mid] = 0.5 # Lower cost mid for peak
    #mask_outer = cand_rad > 6500
    #radial_boost[mask_outer] = 0.2 # Slight boost outer for tail
    ...
    # Print region passing min_bl (before loop)
    min_bl_mask = np.min(d_base, axis=0) >= args.min_bl
    print(f"Region pads passing min_bl: {np.sum(min_bl_mask & (cand_df['type'] == 'region'))}")

    N_r = 24
    r_bins = np.linspace(0, args.fixed_r_max, N_r + 1)  # Fixed bins for consistency
    N_theta = 36
    ...
    ee_hist = ee_hist / 2.0  # Adjust for undirected consistency
    ee_weighted = np.sum(ee_hist)
    cable_dists = np.linalg.norm(cand_pts - existing_pts[0], axis=1)  # Assuming base at existing_pts[0]
    ...
    # Greedy optimization
    sel = []
    var_nn_2d = np.zeros((len(cand_pts), N_cells), dtype=float)  # Initialize empty
    curr_ec_2d = ee_hist.copy()
    curr_nn_2d = np.zeros(N_cells, dtype=float)  # Already in code, but ensure float
    ...
    for i in range(args.n_new):
        ...
        # Target 2D with short-baseline boost
        total_base_new = len(existing_pts) * n_sel
        total_new_new = n_sel * (n_sel - 1) // 2
        total_counts = total_base_new + total_new_new
        #total_base_new = np.sum(ex_weights) * n_sel  # Weighted EN pairs
        #total_new_new = n_sel * (n_sel - 1) / 2.0
        #total_counts = ee_weighted + total_base_new + total_new_new
        #target2d = np.full(N_cells, total_counts / N_cells)
        sigma_b = 10000#8000 # From AR calculation
        ...
        #target2d = np.full(N_cells, total_counts / N_cells)
        #target2d[:N_theta*4] *= 1  # Boost first 4 radial bins for denser short baselines
        #target2d[N_theta*4:N_theta*12] *= 1.5

        # Vectorized cost computation
        fixed_2d = curr_ec_2d + curr_nn_2d
        var_ec_2d = hist_ec_2d

        #if sel:
           # sel_pts = cand_pts[sel]
           # dx = sel_pts[:, 0, None] - cand_pts[None, :, 0]
           # dy = sel_pts[:, 1, None] - cand_pts[None, :, 1]
            #d = np.hypot(dx, dy)
            #phi = (np.arctan2(dy, dx) % (2 * np.pi))
           # ir = np.digitize(d, r_bins) - 1
            #it = np.digitize(phi, theta_bins) - 1
           # valid = (ir >= 0) & (ir < N_r) & (it >= 0) & (it < N_theta)
           # cells = ir * N_theta + it

            #row = np.tile(np.arange(len(cand_pts)), len(sel))[valid.ravel()]
            #col = cells.ravel()[valid.ravel()]
            #data = np.ones(len(col), dtype=int)
            #var_nn_2d = sp.coo_matrix((data, (row, col)), shape=(len(cand_pts), N_cells)).toarray()
       # else:
            #var_nn_2d = np.zeros((len(cand_pts), N_cells), dtype=int)
        #fixed_2d = curr_ec_2d + curr_nn_2d
        #var_ec_2d = hist_ec_2d

        # (the if sel block was here, but removed now)

        fixed_2d = curr_ec_2d + curr_nn_2d
        total_2d = fixed_2d[None, :] + hist_ec_2d + var_nn_2d  # Use accumulated var_nn_2d directly
        err_2d = total_2d - target2d[None, :]  # Add [None, :] to target2d for broadcasting if it's 1D
        J2d_all = np.sum(err_2d ** 2, axis=1)

        #total_2d = fixed_2d[None, :] + var_ec_2d + var_nn_2d
        #err_2d = total_2d - target2d
        #J2d_all = np.sum(err_2d ** 2, axis=1)
        Jc_all = args.cable_weight * cable_dists
        Jtot_all = (J2d_all + Jc_all) * cand_df['cand_weight'].values * radial_boost

        # Soft ratio penalty
        dev_in = max(0, want_in - count_in)  # How much we need more inner
        dev_out = max(0, want_out - count_out)  # How much we need more outer

        # Radial scale factor (1 near center, 0 far out)
        # scale_factor = np.maximum(0, (args.inner_radius - cand_rad) / args.inner_radius)  # Linear decay
        scale_factor = 1 / (1 + np.exp((cand_rad - args.inner_radius) / args.transition_width))  # Sigmoid for smoother

        # Penalty: high for "wrong" additions when scale_factor is high
        is_inner = (cand_region == 'inner')
        penalty = np.zeros(len(cand_pts))
        penalty[is_inner] = scale_factor[is_inner] * (dev_out if dev_in <= 0 else 0)  # Penalize inner if not needed
        penalty[~is_inner] = scale_factor[~is_inner] * (dev_in if dev_out <= 0 else 0)  # Penalize outer if inner needed

        Jtot_all += args.ratio_penalty_weight * penalty

        # Anti-clustering density penalty
        all_neighbors = tree.query_radius(cand_pts, r=args.density_radius)
        neighbor_counts = np.array([len(neighbors) - 1 for neighbors in all_neighbors])
        Jtot_all += args.density_weight * neighbor_counts

        # Masks (no hard region mask)
        ...
        # Incremental update to var_nn_2d: Add directed baselines from all cands to the new pad (bestj)
        k = bestj
        ...
        valid_add = (ir_add >= 0) & (ir_add < N_r) & (it_add >= 0) & (it_add < N_theta) & (d_add > 1e-6)  # Exclude self
        cells_add = ir_add[valid_add] * N_theta + it_add[valid_add]
        m_add = np.where(valid_add)[0]  # Indices of valid candidates
        np.add.at(var_nn_2d, (m_add, cells_add), 1.0)  # Add 1.0 (or new pad weight if not 1)
        # Ban neighbors
        bad = neighbors[bestj]
        allowed[bad] = False
        allowed[bestj] = True  # Ensure self is allowed (though not needed)

        logging.info(f"Step {i+1}/{args.n_new}: added {bestj}, cost={bestJ:.1f}")

        # Dynamic angular every 5 steps
        if (i + 1) % 5 == 0:
            current_existing = np.vstack([existing_pts, cand_pts[sel[:-1]]]) if sel[:-1] else existing_pts  # All prior as "existing"
    ...
    bl_bins = np.linspace(0, args.fixed_r_max, 65)#bl_bins = np.linspace(0, args.fixed_r_max, 65)  # Use fixed_r_max for consistent bins in plot too
    bl = pdist(all_pads[['x', 'y']].values)

    Rmax = args.fixed_r_max               # your max baseline

    # call our new function
    plot_normalized_histogram(
        baselines=bl,
        R_max=Rmax,
        nbins=65,                         # choose your bin count
        title=f"Norm. Basline distribution",
        filename="baseline_norm_hist.png" # saved to disk
    )
